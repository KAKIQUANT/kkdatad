...
from .settings import TushareIntegrationSettings
    ...
    def __init__(self, settings: TushareIntegrationSettings):
        ...
        self.templates = {
            'create': jinja2.Template(
                open(
                    f'kkdatad/schema/template/{self.settings.database.db_type.lower()}/table.jinja2'
                ).read()
            ),
            'insert': jinja2.Template(
                open(
                    f'kkdatad/schema/template/{self.settings.database.db_type.lower()}/insert.jinja2'
                ).read()
            ),
            'upsert': jinja2.Template(
                open(
                    f'kkdatad/schema/template/{self.settings.database.db_type.lower()}/upsert.jinja2'
                ).read()
            ),
        }
...
class DatabaseEngineFactory(object):
    ...
    def create(settings: TushareIntegrationSettings) -> DBEngine:
        ...
        elif settings.database.db_type == 'dolphin':
            raise NotImplementedError
        else:
            raise NotImplementedError
